...
Camera1P::Camera1P(const glm::vec3& position, const float& pitch, const float& yaw, const float& roll): position(position), pitch(glm::radians(pitch)), yaw(glm::radians(yaw)), roll(glm::radians(roll)), view(1.0) {}

Camera1P::Camera1P(const Camera1P& camera): position(camera.position), pitch(camera.pitch), yaw(camera.yaw), roll(camera.roll), view(camera.view) {}

Camera1P::Camera1P(Camera1P&& camera): position(std::move(camera.position)), pitch(std::move(camera.pitch)), yaw(std::move(camera.yaw)), roll(std::move(camera.roll)), view(std::move(camera.view)) {}
...
glm::vec3& Camera1P::getPosition() { return position; }
...
const glm::mat4& Camera1P::getViewMatrix() {
